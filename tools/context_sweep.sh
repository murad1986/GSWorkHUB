...
# 1. Календарь — единственный источник с собственным коннектором. Нет доступа —
#    шаг молчит: отсутствие ключа не должно ронять весь обход.
if [ "$DRY" = "0" ]; then
    make -s calendar >>"$LOG" 2>&1 || say "приём календаря не прошёл — см. лог"
fi
