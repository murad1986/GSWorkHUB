...
Склад умеет сам ходить в календарь, задачи и Granola — у них есть собственные
коннекторы. Всё остальное — диктофон Plaud, переписка, документы, доски —
доступно только агенту через MCP-серверы. Этот документ описывает, как обе
половины запускаются одной командой и как поставить их на расписание.
...
1. **Свои коннекторы** — `make sources` (календарь, TickTick, склад) и
   `make granola`, если приложение стоит. Источник без доступа молчит и не
   роняет остальной обход.
